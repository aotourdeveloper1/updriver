...
Route::post('/login', function (Request $request) {
    ...
    return Response::json([
        'response' => true,
        'servicios' => $servicios,
        'test' => $request->servicios,
        'test2' => $request->prueba
    ]);
    ...
    foreach ($servicios as $cli) {
        ...
        $viaje->fk_centrodecosto = $cli['centrodecosto_id'];
        $viaje->fk_subcentrodecosto = $cli['subcentrodecosto_id'];
        $viaje->fk_departamento = $dep->id;
        $viaje->fk_ciudad = $ciu->id;
        $viaje->solicitante = $cli['solicitado_por'];
        $viaje->email_solicitante = $cli['email_solicitante'];
        $viaje->fk_sede = 2;
        $viaje->fecha_solicitud = $cli['fecha_solicitud'];
        $viaje->tipo_servicio = 44;
        $viaje->tipo_traslado = 70;
        $viaje->tipo_ruta = $tipe;
        $viaje->fk_traslado = $cli['ruta_id'];
        $viaje->detalle_recorrido = $cli['detalle_recorrido'];
        $viaje->fk_proveedor = $cli['proveedor_id'];
        $viaje->fk_conductor = $cli['conductor_id'];
        $viaje->fk_vehiculo = $cli['vehiculo_id'];
        $viaje->fecha_viaje = $cli['fecha_servicio'];
        $viaje->hora_viaje = $cli['hora_servicio'];

        $viaje->creado_por = 2;

        $viaje->control_facturacion = $cli['control_facturacion'];
        $viaje->fk_estado = 57;

        $viaje->save();

        $destino = new Destino;
        $destino->direccion = $cli['recoger_en'];
        $destino->fk_viaje = $viaje->id;
        $destino->orden = 1;
        $destino->save();

        $destino = new Destino;
        $destino->direccion = $cli['dejar_en'];
        $destino->fk_viaje = $viaje->id;
        $destino->orden = 2;
        $destino->save();

        $passs = json_decode($cli['pasajeros_ruta']);

        foreach ($passs as $paa) {

            if($paa->area=='RUTA EJECUTADA CON ÉXITO') {
                $eestadoo = 87;
            }else if($paa->area=='NO SE PRESENTÓ'){
                $eestadoo = 88;
            }else if($paa->area=='CONSULTOR NO PROGRAMADO'){
                $eestadoo = 89;
            }else if($paa->area=='CONSULTOR TOMA RUTA EN HORARIO NO PROGRAMADO'){
                $eestadoo = 90;
            }else if($paa->area=='NO SHOW'){
                $eestadoo = 91;
            }else{
                $eestadoo = 86;
            }

            $pasajero2 = new PasajeroRutaQr;
            $pasajero2->id_empleado = $paa->apellidos;
            $pasajero2->nombre = $paa->nombres;
            $pasajero2->programa = $paa->sub_area;
            $pasajero2->celular = $paa->cedula;
            $pasajero2->direccion = $paa->direccion;
            $pasajero2->barrio = $paa->barrio;
            $pasajero2->localidad = $paa->cargo;
            $pasajero2->estado_ruta = $eestadoo;
            $pasajero2->fk_viaje = $viaje->id;
            $pasajero2->save();
